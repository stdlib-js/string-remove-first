...
function removeFirst( str ) {
	...
	if ( !isString( str ) ) {
		throw new TypeError( format( '1P73F', str ) );
	}
	opts = {
		'mode': 'grapheme'
	};
	...
	if ( nargs === 1 ) {
		...
		if ( isPlainObject( n ) ) {
			...
			throw new TypeError( format( '1P73X', n ) );
		}
	} else { // nargs > 2
		n = arguments[ 1 ];
		...
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '1P72V', options ) );
		}
	}
	if ( options ) {
		if ( hasOwnProp( options, 'mode' ) ) {
			...
			if ( !isMode( opts.mode ) ) {
				throw new TypeError( format( '1P7E1', 'mode', MODES.join( '", "' ), opts.mode ) );
			}
		}
	}
